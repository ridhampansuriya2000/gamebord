...
export default function ReverseCard({ card, onClick, selectable, selected, disabled, hidden }) {
  if (hidden) {
    return (
      <div className="w-16 h-24 sm:w-20 sm:h-28 bg-gradient-to-br from-slate-800 to-black rounded-lg sm:rounded-xl border-[2px] sm:border-4 border-white shadow-[0_4px_10px_rgba(0,0,0,0.5)] flex items-center justify-center relative overflow-hidden group">
  ...
  return (
    <button
      ...
      className={`relative w-16 h-24 sm:w-20 sm:h-28 ${bgColor} rounded-lg sm:rounded-xl border-[2px] sm:border-[3px] border-white overflow-hidden transition-all duration-300 transform-gpu
