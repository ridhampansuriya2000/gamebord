...
export default function PlayingCard({ card, onClick, onMouseEnter, selectable, selected, hidden, disabled }) {
  if (hidden) {
    return (
  ...
  return (
    <button
      ...
      onMouseEnter={() => selectable && onMouseEnter && onMouseEnter(card)}
    ...
    >
      ...
      {isFaceCard ? (
        <div className={`absolute inset-0 flex items-center justify-center pointer-events-none opacity-100 ${colorClass}`}>
          <div className="text-[12px] sm:text-xl leading-none">{suitSymbols[card.suit]}</div>
        </div>
      ) : (
        <div className="absolute top-[18%] bottom-[18%] left-[20%] right-[20%] opacity-100 pointer-events-none">
          {pips.map((pos, i) => (
            <div key={i} className={`absolute ${pipPos[pos]} text-[12px] sm:text-xl leading-none ${colorClass}`}>
              {suitSymbols[card.suit]}
            </div>
          ))}
        </div>
